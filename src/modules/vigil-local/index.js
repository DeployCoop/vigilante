...
import { loadConfig, saveConfig } from '../../engine/config.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

export const DEFAULT_VIGIL_LOCAL_CHART_PATH = '/opt/vigil/infra/helm/vigil';
...
export class VigilLocalModule extends BaseModule {
  ...
  /**
   * Get dynamic module description reflecting current local chart path
   */
  get description() {
    const curPath = this.getChartPath();
    return `Vigil Autonomous AI-Native SOC Platform installed directly from local checkout at ${curPath}`;
  }
  ...
  /**
   * Get configured local Vigil Helm chart path
   * @returns {string}
   */
  getChartPath() {
    const cfg = loadConfig();
    return process.env.VIGIL_LOCAL_CHART_PATH ||
  ...
  /**
   * Update configured local Vigil Helm chart path in config.yaml
   * @param {string} newPath
   * @returns {Promise<{ path: string, exists: boolean }>}
   */
  async setChartPath(newPath) {
    if (!newPath || typeof newPath !== 'string') {
      throw new Error('Chart path must be a non-empty string');
    }
    const cleanInput = newPath.trim();
    const expanded = cleanInput.startsWith('~') ? path.join(os.homedir(), cleanInput.slice(1)) : cleanInput;
    ...
    let exists = false;
    try {
      await fs.access(path.join(resolved, 'Chart.yaml'));
      exists = true;
    } catch {
      exists = false;
    }
  ...
  async resolveLocalChartPath(options = {}) {
    ...
    const expanded = candidatePath.startsWith('~') ? path.join(os.homedir(), candidatePath.slice(1)) : candidatePath;
    const resolved = path.resolve(expanded);
  ...
  /**
   * Uninstall vigil-local and optionally delete its namespace
   */
