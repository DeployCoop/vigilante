...
const AppContent = ({
  ...
  const [openvasTargetHost, setOpenvasTargetHost] = useState(null);
  ...
  useInput((input, key) => {
    ...
    // Trigger OpenVAS / Greenbone Vulnerability Scanner
    if (keyChar === 'o') {
      logger.info('UI:ACTION', 'User pressed [o] -> Switching to OPENVAS view');
      setFatalError(null);
      setViewState('OPENVAS');
      return;
    }

    // Trigger kCTF Cyber Range & Challenges
    if (keyChar === 'f') {
  ...
  // -------------------------------------------------------------
  // Command: UNINSTALL MODULE (Individual Module Teardown)
  // -------------------------------------------------------------
  const runUninstallModuleWorkflow = async ({ moduleId, namespace: customNs = null, deleteNamespace: deleteNs = false }) => {
    const activeNs = customNs || targetNamespace || 'default';
    const mod = globalModuleRegistry.get(moduleId);
    const modName = mod ? mod.name : moduleId;

    logger.info('WORKFLOW:UNINSTALL_MODULE', `Uninstalling module '${moduleId}' from namespace '${activeNs}'`);
    setViewState('RUNNING');
    setLogs([]);
    setFatalError(null);
    setIsDone(false);
    ...
    setTasks([
      {
        id: `uninst-${moduleId}`,
        label: `Uninstall package: ${modName}`,
        status: 'running'
      }
    ]);

    try {
      addLog(`Initiating uninstallation of '${modName}' from namespace '${activeNs}'...`);
      await globalModuleRegistry.uninstallModule({
        moduleId,
        namespace: activeNs,
        clusterName,
      ...
      updateTask(`uninst-${moduleId}`, { status: 'done' });
      addLog(`✔ Successfully uninstalled '${modName}' from namespace '${activeNs}'!`);
      setIsDone(true);
      setViewState('SUCCESS');
    } catch (err) {
      logger.error('WORKFLOW:UNINSTALL_MODULE:ERROR', err.message, err);
      updateTask(`uninst-${moduleId}`, { status: 'error' });
      setFatalError(err.message);
      setIsDone(true);
    }
  };
  ...
  // -------------------------------------------------------------
  // Command: RESET MODULE (Clean In-Place Reinstall)
  // -------------------------------------------------------------
  const runResetModuleWorkflow = async ({ moduleId, namespace: customNs = null }) => {
    const activeNs = customNs || targetNamespace || 'default';
    const mod = globalModuleRegistry.get(moduleId);
    const modName = mod ? mod.name : moduleId;

    logger.info('WORKFLOW:RESET_MODULE', `Resetting module '${moduleId}' in namespace '${activeNs}'`);
    setViewState('RUNNING');
    setLogs([]);
    setFatalError(null);
    setIsDone(false);
    ...
    setTasks([
      {
        id: `reset-uninst-${moduleId}`,
        label: `Tear down existing ${modName} resources`,
        status: 'running'
      },
      {
        id: `reset-inst-${moduleId}`,
        label: `Reinstall clean ${modName} deployment`,
        status: 'pending'
      }
    ]);

    try {
      addLog(`Resetting '${modName}' in namespace '${activeNs}'...`);
      addLog(`Phase 1/2: Uninstalling existing resources...`);
      await globalModuleRegistry.uninstallModule({
        moduleId,
        namespace: activeNs,
        clusterName,
      ...
      updateTask(`reset-uninst-${moduleId}`, { status: 'done' });

      updateTask(`reset-inst-${moduleId}`, { status: 'running' });
      addLog(`Phase 2/2: Reinstalling clean instance of '${modName}'...`);
      const certs = await setupCertificates(domain, { clusterName, instanceName: clusterName });
      await ensureNamespace(activeNs, { onLog: (msg) => addLog(msg) });

      await globalModuleRegistry.resetModule({
        moduleId,
        domain,
        namespace: activeNs,
        clusterName,
        certPath: certs?.certPath,
        keyPath: certs?.keyPath,
        options: { customValuesPath, customValuesDir },
        onLog: (msg) => addLog(msg)
      });

      updateTask(`reset-inst-${moduleId}`, { status: 'done' });
      addLog(`✔ Successfully reset and reinstalled '${modName}' in namespace '${activeNs}'!`);
      setIsDone(true);
      setViewState('SUCCESS');
    } catch (err) {
      logger.error('WORKFLOW:RESET_MODULE:ERROR', err.message, err);
      updateTask(`reset-inst-${moduleId}`, { status: 'error' });
      setFatalError(err.message);
      setIsDone(true);
    }
  };
  ...
  useEffect(() => {
    ...
    } else if (command === 'reset' || (command === 'modules' && subCommand === 'reset')) {
      const targetMod = (cliSelectedModules && cliSelectedModules[0]) || 'vigil-soc';
      runResetModuleWorkflow({ moduleId: targetMod, namespace: targetNamespace });
    } else {
  ...
  return React.createElement(
    ...
    viewState === 'MODULES'
      ? React.createElement(ModulesView, {
          ...
          onUninstall: ({ moduleId, namespace: uninstNs }) => {
            runUninstallModuleWorkflow({ moduleId, namespace: uninstNs, deleteNamespace: false });
          },
          onReset: ({ moduleId, namespace: resetNs }) => {
            runResetModuleWorkflow({ moduleId, namespace: resetNs });
          },
    ...
    viewState === 'XML_VISUALIZER'
      ? React.createElement(NmapVisualizerView, {
          ...
          onNavigate: (target, optArg) => {
            if (target === 'openvas') {
              if (optArg) setOpenvasTargetHost(optArg);
              setViewState('OPENVAS');
            } else if (target === 'nmap') {
              setViewState('NMAP');
            } else if (target === 'status') {
    ...
    // State 12: OpenVAS / Greenbone Vulnerability Scanner View
    viewState === 'OPENVAS'
      ? React.createElement(OpenVASView, {
          domain,
          clusterName,
          namespace: targetNamespace === 'default' ? 'openvas' : targetNamespace,
          initialTargetHost: openvasTargetHost,
          onNavigate: (target, optArg) => {
            if (target === 'menu') {
              setPreviousViewState('OPENVAS');
              setViewState('MENU');
            } else if (target === 'nmap') {
              setViewState('NMAP');
            } else if (target === 'xml-visualizer') {
              setSelectedXmlPath(optArg);
              setViewState('XML_VISUALIZER');
            } else if (target === 'pods') {
              setViewState('PODS');
    ...
    // State 13: kCTF Cyber Range & Challenges View
    viewState === 'KCTF'
      ? React.createElement(KCTFView, {
          domain,
          clusterName,
          namespace: targetNamespace === 'default' ? 'kctf' : targetNamespace,
          onNavigate: (target, optArg) => {
            if (target === 'menu') {
              setPreviousViewState('KCTF');
              setViewState('MENU');
            } else if (target === 'dashboard') {
              if (dashboardData) {
                setViewState('DASHBOARD');
