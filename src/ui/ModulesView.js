...
export const ModulesView = memo(function ModulesView({
  ...
  const handleSingleUninstall = async (targetMod) => {
    if (!targetMod || isProcessingModule) return;
    setIsProcessingModule(true);
    setFeedback({
      type: 'info',
      text: `⏳ Uninstalling module '${targetMod.name}' from namespace '${currentTargetNamespace}'...`
    });

    try {
      if (onUninstall) {
        await onUninstall({ moduleId: targetMod.id, namespace: currentTargetNamespace });
      } else {
      ...
      setFeedback({
        type: 'success',
        text: `✔ Module '${targetMod.name}' successfully uninstalled from '${currentTargetNamespace}'.`
      });
      await loadModules(currentTargetNamespace);
    } catch (err) {
      setFeedback({
        type: 'error',
        text: `✖ Failed to uninstall '${targetMod.name}': ${err.message}`
      });
    } finally {
  ...
  const handleSingleReset = async (targetMod) => {
    if (!targetMod || isProcessingModule) return;
    setIsProcessingModule(true);
    setFeedback({
      type: 'info',
      text: `⏳ Resetting & reinstalling '${targetMod.name}' in namespace '${currentTargetNamespace}'...`
    });

    try {
      if (onReset) {
        await onReset({ moduleId: targetMod.id, namespace: currentTargetNamespace });
      } else {
      ...
      setFeedback({
        type: 'success',
        text: `✔ Module '${targetMod.name}' successfully reset and running in '${currentTargetNamespace}'.`
      });
      await loadModules(currentTargetNamespace);
    } catch (err) {
      setFeedback({
        type: 'error',
        text: `✖ Failed to reset '${targetMod.name}': ${err.message}`
      });
    } finally {
      setIsProcessingModule(false);
    }
  ...
  // Keyboard navigation, interactive namespace editing & chart path editing
  useInput((input, key) => {
    ...
    // Mode B: Editing Target Namespace
    ...
    // [p] or [P] -> Trigger Local Vigil Chart Path Prompt if focused on vigil-local (or if vigil-local exists)
    if (keyChar === 'p') {
    ...
    // [x] -> Uninstall focused module
    if (keyChar === 'x') {
      const currentMod = modules[cursor];
      if (currentMod) {
        handleSingleUninstall(currentMod);
      }
      return;
    }

    // [r] -> Reset and reinstall focused module
    if (keyChar === 'r') {
      const currentMod = modules[cursor];
      if (currentMod) {
        handleSingleReset(currentMod);
      }
      return;
    }
  ...
  return React.createElement(
      ...
      : isEditingNamespace
        ...
        : React.createElement(
            ...
            React.createElement(
              ...
              isCurrentVigilLocal
                ? 'Use ↑/↓ to navigate, [Space] to toggle, [p] to edit local chart path, [n] for namespace, [Enter] to apply.'
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(Text, { color: theme.error || 'red', bold: true }, '[x] '),
        React.createElement(Text, { color: theme.text || 'white' }, 'Uninstall  '),
        React.createElement(Text, { color: theme.primary || 'cyan', bold: true }, '[r] '),
        React.createElement(Text, { color: theme.text || 'white' }, 'Reset  '),
