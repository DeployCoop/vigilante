...
import { runHostVulnerabilityScan } from '../engine/openvas.js';
...
export const NmapVisualizerView = memo(function NmapVisualizerView({
  ...
  const executeOpenVas = async (host) => {
    if (!host) return;
    if (onNavigate) {
      onNavigate('openvas', host.ip);
      return;
    }
    setActiveDiagnostic({ tool: '🛡️ OpenVAS Vulnerability Scan', host: host.ip, isRunning: true, output: '' });
    const res = await runHostVulnerabilityScan({
      host,
      networkTarget
    });
    const updatedEvidence = await listHostEvidence(networkTarget, host.ip);
    ...
    const summary = [
      `=== 🛡️ OpenVAS / Greenbone Vulnerability Audit ===`,
      `Target: ${host.ip} (${res.summary.totalFindings} findings)`,
      `Severity: ${res.summary.overallSeverity} (Max CVSS: ${res.summary.maxCvss})`,
      `Critical: ${res.summary.critical} | High: ${res.summary.high} | Medium: ${res.summary.medium} | Low: ${res.summary.low}`,
      '',
      `Findings:`,
      ...res.findings.map(f => `  • [${f.severity}] ${f.cve}: ${f.title} (${f.service}:${f.port})`),
      '',
      `Remediation details and full report saved to:`,
      `evidence/${networkTarget.replace(/\//g, '_')}/${host.ip}/openvas_report.json`
    ].join('\n');

    setActiveDiagnostic({
      tool: 'OpenVAS Vulnerability Scan',
      host: host.ip,
      isRunning: false,
      command: `openvas-scan --target ${host.ip}`,
      output: summary,
      success: true,
      savedFile: `evidence/${networkTarget.replace(/\//g, '_')}/${host.ip}/openvas_report.json`
    });
  };
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(Text, { color: theme.error, bold: true }, '[o] '),
        React.createElement(Text, { color: theme.text }, 'OpenVAS  '),
